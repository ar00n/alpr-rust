...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fps: u32 =
        sqlx::query_scalar!("SELECT value FROM settings WHERE key = 'processing_framerate'")
            .fetch_optional(&db)
            .await?
    ...
    let trim_snapshots_mb: Option<u64> =
        sqlx::query_scalar!("SELECT value FROM settings WHERE key = 'trim_snapshots_mb'")
            .fetch_optional(&db)
            .await?
            .map(|v| v.parse::<u64>().unwrap_or(0));

    let trim_history_days: Option<u64> =
        sqlx::query_scalar!("SELECT value FROM settings WHERE key = 'trim_history_days'")
            .fetch_optional(&db)
            .await?
            .map(|v| v.parse::<u64>().unwrap_or(0));

    let min_confidence: f32 =
        sqlx::query_scalar!("SELECT value FROM settings WHERE key = 'min_confidence'")
            .fetch_optional(&db)
            .await?
            .and_then(|v| v.parse::<f32>().ok())
            .unwrap_or(0.8);

    let (pipeline_config_tx, pipeline_config_rx) =
        watch::channel::<PipelineConfig>(PipelineConfig {
            fps,
            rtsp_url,
            trim_snapshots_mb,
            trim_history_days,
    ...
    let rtsp_handle = tokio::spawn(async move {
        if let Err(e) = start_rtsp_ingest(
            tx_clone,
            pipeline_tx_clone,
            pipeline_config_rx_rtsp,
            rtsp_shutdown_rx,
        )
        .await
        {

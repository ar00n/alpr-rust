...
pub fn get_or_create_key() -> Result<Vec<u8>, AppError> {
    if let Ok(env_key) = env::var("ENCRYPTION_KEY") {
        ...
        let key_bytes = STANDARD.decode(trimmed).map_err(|_| {
            AppError::internal("Invalid Base64 in ENCRYPTION_KEY environment variable")
        })?;
    ...
    if key_path.exists() {
        ...
        if key_bytes.len() != 32 {
            return Err(AppError::internal(
                "Key file does not contain a valid 32-byte key",
            ));
        }
        ...
        options
            .open(key_path)
            .map_err(|_| AppError::internal("Failed to open encryption key file"))?
...
pub fn encrypt_data(plaintext: &str, key: &[u8]) -> Result<String, AppError> {
    let cipher =
        Aes256Gcm::new_from_slice(key).map_err(|_| AppError::internal("Invalid key length"))?;
